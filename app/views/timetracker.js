...
app.TimeTrackerView = Backbone.View.extend({
    ...
    render: function() {
        jQuery('#t_main').html(this.timeTemplate({ t_total_slots: 5 }));
    },
    ...
    updateCategorySelect: function() {
        var category_select_html = _.template(jQuery('#category-select-template').html(), {
    ...
    t_add_slot_input: function( event ) {
        if ( event.which !== ENTER_KEY || !jQuery('#t_description').trim() ) {
            return;
        }
    ...
    create_time_slot: function() {
        app.TimeSlots.create( {
            ...
            t_duration: parseInt(jQuery('#t_duration').val().trim(), 10),
            t_category: jQuery('#t_category').val().trim(),
            t_project: jQuery('#t_project').val().trim(),
            t_task: jQuery('#t_task').val().trim(),
    ...
    t_select_project: function () {
        var task_select_html = _.template(jQuery('#task-select-template').html(), {
            tasks : app.Tasks.toJSON(),
            project: jQuery('#t_project').val().trim()
        });
        jQuery('#t_task').html(task_select_html);
    },
